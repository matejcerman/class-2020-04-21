...
logistic_fit <- fit(logistic_mod,
                   factor(trump) ~ gender + income + race + age + educ + ideology + party,
                   data = cces)
...
tree_fit <- fit(tree_mod,
                factor(trump) ~ gender + income + race + age + educ + ideology + party,
                data = cces)

tree_fit_complex <- fit(update(tree_mod, cost_complexity = 0.001),
                        factor(trump) ~ gender + income + race + age + educ + ideology + party,
                        data = cces)

tree_fit_complex$fit %>%
  prp(extra = 'auto')
...
```{r}
logistic_preds <- predict(logistic_fit,
                          new_data = cces)

tree_preds <- predict(tree_fit,
                      new_data = cces)

forest_preds <- predict(forest_fit,
                        new_data = cces)
...
predictions <- cces %>%
  bind_cols(logistic_preds,
            tree_preds,
            forest_preds) %>%
  rename(log_pred = .pred_class,
         tree_pred = .pred_class1,
         forest_pred = .pred_class2)

logistic_accuracy <- predictions %>%
  accuracy(truth = trump, estimate = log_pred)
tree_accuracy <- predictions %>%
  accuracy(truth = trump, estimate = tree_pred)
forest_accuracy <- predictions %>%
  accuracy(truth = trump, estimate = forest_pred)
...
acc <- logistic_accuracy %>%
  bind_rows(tree_accuracy) %>%
  bind_rows(forest_accuracy) %>%
